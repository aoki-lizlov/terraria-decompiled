...
namespace FullSerializer.Internal
{
	...
	public class fsDateConverter : fsConverter
	{
		...
		public override fsResult TryDeserialize(fsData data, ref object instance, Type storageType)
		{
			...
			{
				DateTime dateTime;
				if (DateTime.TryParse(data.AsString, null, 128, ref dateTime))
				{
					instance = dateTime;
					return fsResult.Success;
				}
				if (fsGlobalConfig.AllowInternalExceptions)
				{
					...
					{
						return fsResult.Fail(string.Concat(new object[] { "Unable to parse ", data.AsString, " into a DateTime; got exception ", ex }));
					}
				}
			...
			else if (storageType == typeof(DateTimeOffset))
			{
				...
				if (DateTimeOffset.TryParse(data.AsString, null, 128, ref dateTimeOffset))
				{
			...
			else
			{
				...
				TimeSpan timeSpan;
				if (TimeSpan.TryParse(data.AsString, ref timeSpan))
				{
					instance = timeSpan;
					return fsResult.Success;
				}
		...
		// Token: 0x060000CB RID: 203 RVA: 0x00005B35 File Offset: 0x00003D35
