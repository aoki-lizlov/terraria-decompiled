...
namespace Terraria.GameContent.UI
{
	...
	{
		// Token: 0x06002940 RID: 10560 RVA: 0x005795CC File Offset: 0x005777CC
		public void Draw()
		{
			...
			for (int i = 0; i < 255; i++)
			{
				if (player[i].active && myPlayer != i && (!player[i].dead || player[i].ghost) && player[myPlayer].team > 0 && player[myPlayer].team == player[i].team)
				{
					...
					Color color = new Color((int)((byte)((float)teamColor[player[i].team].R * num4)), (int)((byte)((float)teamColor[player[i].team].G * num4)), (int)((byte)((float)teamColor[player[i].team].B * num4)), (int)mouseTextColor);
					...
					if (num3 > 0f)
					{
						...
						if (num11 <= (float)num)
						{
							...
							LegacyMultiplayerClosePlayersOverlay.DrawPlayerName2(spriteBatch, ref color, textValue, ref vector5);
							Color playerHeadBordersColor = Main.GetPlayerHeadBordersColor(player[i]);
							Vector2 vector6 = new Vector2(vector.X, vector.Y - num9);
							vector6.X -= 22f + num10;
							vector6.Y += 8f;
							playerRenderer.DrawPlayerHead(camera, player[i], vector6, 1f, 0.8f, playerHeadBordersColor);
							Vector2 vector7 = vector5 + screenPosition2 + new Vector2(26f, 20f);
							if (player[i].statLife != player[i].statLifeMax2)
							{
								Main.instance.DrawHealthBar(vector7.X, vector7.Y, player[i].statLife, player[i].statLifeMax2, 1f, 1.25f, true);
							}
							ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.MouseText.Value, name, vector + new Vector2(0f, -40f), color, 0f, Vector2.Zero, Vector2.One, -1f, 2f);
						}
					}
					else
					{
						LegacyMultiplayerClosePlayersOverlay.DrawPlayerName(spriteBatch, name, ref vector, ref color);
					}
				}
			}
		}

		// Token: 0x06002941 RID: 10561 RVA: 0x00579BB0 File Offset: 0x00577DB0
		...
		// Token: 0x06002942 RID: 10562 RVA: 0x00579D08 File Offset: 0x00577F08
		...
		// Token: 0x06002943 RID: 10563 RVA: 0x0000357B File Offset: 0x0000177B
